...
def denoising(noise_im, clean_im, LR=1e-2, sigma=3, rho=1, eta=0.5, total_step=30, 
              prob1_iter=500, noise_level=None, result_root=None, f=None, net=None):
    input_depth = 3
    latent_dim = 3
    ...
    model_path = './model_zoo/ffdnet_color.pth'
...
################################# sub-problem 2 ###############################
            ...
            sig_tensor = torch.full((1,1,1,1), sig/255.).type_as(i0)
            i0_til_torch = model((i0 + Y)/255, sig_tensor) * 255
...
###############################################################################
            ...
            print('Iteration: %05d, VAE Loss: %f, PSNR: %f, SSIM: %f' % (i, total_loss.item(), psnr, ssim), file=f, flush=True)
...
if __name__ == "__main__":
    ...
    for noise, clean in zip(noises, cleans):
        result = './output/nn_ffd_cc/{}/'.format(noise.split('/')[-1][:-9])
        ...
        with open(result + 'result.txt', 'w') as f:
            _, psnr, ssim = denoising(noise_im_np, clean_im_np, LR=LR, sigma=sigma, 
                                      ...
                                      result_root=result, f=f, net=net)

            psnrs.append(psnr)
            ssims.append(ssim)
    with open('./output/nn_ffd_cc/' + 'psnr_ssim.txt', 'w') as f:
