...
def denoising(noise_im, clean_im, LR=1e-2, sigma=5, rho=1, eta=0.5, total_step=30, 
...
################################# sub-problem 2 ###############################
            ...
            i0_til_np = denoise_nl_means(i0_np.transpose(1, 2, 0) + Y_np.transpose(1, 2, 0), 
                                         h=0.8*sig, sigma=sig, fast_mode=False, **patch_kw).transpose(2, 0, 1)
...
if __name__ == "__main__":
    ...
    for noise, clean in zip(noises, cleans):
        result = './output/nn_nlm_polyu/{}/'.format(noise.split('/')[-1][:-9])
        ...
        with open(result + 'result.txt', 'w') as f:
            ...
            psnrs.append(psnr)
            ssims.append(ssim)
    with open('./output/nn_nlm_polyu/' + 'psnr_ssim.txt', 'w') as f:
