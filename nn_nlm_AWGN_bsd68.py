...
def denoising_gray(noise_im, clean_im, LR=1e-2, sigma=5, rho=1, eta=0.5, 
    ...
    sig = noise_level 
    r_nlm = denoise_nl_means(noise_im, h=0.8 * sig, sigma=sig,
                             fast_mode=False, **patch_kw)
    r_nlm = np.clip(r_nlm, 0, 1)

    psnr_nlm = compare_psnr(clean_im, r_nlm, 1)
    ssim_nlm = compare_ssim(r_nlm, clean_im, multichannel=False, data_range=1)
    print('PSNR_NLM: {}, SSIM_NLM: {}'.format(psnr_nlm, ssim_nlm), file=fo, flush=True)
    
    r_nlm = Image.fromarray((255*r_nlm).astype(np.uint8))
...
############################### sub-problem 2 #################################
            ...
            i0_til_np = denoise_nl_means(tmp, h=0.8 * sig, sigma=sig,
                                         fast_mode=False, **patch_kw)
...
if __name__ == "__main__":
    ...
    for noise_level in noise_levels:
        ...
        for clean_root in clean_roots:
            result = './output/nn_nlm_AWGN_BSD68/Gaussian_{}/{}/'.format(noise_level, clean_root.split('/')[-1][:-4])
        ...
        result = './output/nn_nlm_AWGN_BSD68/Gaussian_{}/'.format(noise_level)
        with open(result + 'psnr_ssim.txt', 'w') as f:
            ...
            print('avr nlm psnr =', sum(bm3d_psnr)/len(bm3d_psnr), file=f, flush=True)
            print('avr nlm ssim =', sum(bm3d_ssim)/len(bm3d_ssim), file=f, flush=True)
