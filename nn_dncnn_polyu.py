...
def denoising(noise_im, clean_im, LR=1e-2, sigma=5, rho=1, eta=0.5, 
    ...
    psnr_dncnn = compare_psnr(clean_im.transpose(1, 2, 0), r_dncnn_np.transpose(1, 2, 0), 1)
    ssim_dncnn = compare_ssim(r_dncnn_np.transpose(1, 2, 0), clean_im.transpose(1, 2, 0), multichannel=True, data_range=1)
    
    print('PSNR_DNCNN: {}, SSIM_DNCNN: {}'.format(psnr_dncnn, ssim_dncnn), file=f, flush=True)
...
if __name__ == "__main__":
    ...
    for noise, clean in zip(noises, cleans):
        result = './output/nn_dncnn_polyu/{}/'.format(noise.split('/')[-1][:-9])
        ...
        with open(result + 'result.txt', 'w') as f:            
            _, best_psnr, best_ssim = denoising(noise_im_np, clean_im_np, LR=LR, sigma=sigma, rho=rho, eta=eta, 
    ...
    with open('./output/nn_dncnn_polyu/' + 'psnr.txt', 'w') as f:
